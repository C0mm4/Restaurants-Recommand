...
def overlapped_data(start_x, start_y, end_x, end_y):
    # 최종 데이터가 담길 리스트
    overlapped_result = []
    ...
    for i in np.linspace(start_x, end_x, 10):  
        ...
        for j in np.linspace(start_y, end_y, 10):
            target_y = initial_start_y + i
            each_result= whole_region(start_x,initial_start_y,target_x,target_y)
            overlapped_result.extend(each_result)
            initial_start_y = target_y
        start_x = target_x
...
datas = overlapped_data(start_x, start_y, end_x, end_y)
